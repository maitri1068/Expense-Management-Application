...
export default function Settings() {
  ...
  async function handleuserupdate(val){
    ...
    try{
        const i=await axios.patch('http://localhost:1010/updateprofile',{data:val}, {
          headers: {
  ...
  const formik = useFormik({
    ...
    onSubmit: (values) => {
     console.log("values",{...values,_id:user._id})
...
<Box container sx={{m:3,p:2,maxWidth:600}}><form onSubmit={formik.handleSubmit}>
                    ...
                    <Button type='submit' variant='contained' sx={{display:( open )?'block':'none',mt:2}} onClick={()=>{
