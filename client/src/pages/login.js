...
import {TextField,FormControl,InputAdornment,InputLabel,
  IconButton,
  Container,
  OutlinedInput,
  Button,
...
export default function Login() {
  ...
  const handleloginform = async (data) => {
    try {
      const i = await axios.post("http://localhost:1011/login", data);
      ...
      if (i.data === "you don't have account please signup first") {
        ...
        try {
          ...
          const d = await axios.post("http://localhost:1011/loginn", data);
  ...
  const handleragistrationform = async (data) => {
    try {
      const i = await axios.post("http://localhost:1011/finduser", data);

      if (i.status === 201) {
        try {
          const otp = await axios.post(
            "http://localhost:1011/varifymail",
            data
          );
  ...
  const handleotp = async (data) => {
    try {
      ...
      const i = await axios.post("http://localhost:1011/passwordreset", data);
  ...
  const updatepassword = async (data) => {
    ...
    try {
      const i = axios.patch("http://localhost:1011/updatepassword", {
  ...
  const handleregotp = async (data) => {
    ...
    if (op) {
      ...
      if (data.regotp == op.otp) {
        try {
          const i = await axios.post("http://localhost:1011/user", userinfo);
  ...
  async function handleresetotp() {
    ...
    try {
      ...
      if (JSON.parse(localStorage.getItem("form")) === 6) {
        const i = await axios.post("http://localhost:1011/varifymail", {
          email: email,
        });
        localStorage.setItem("regotp", JSON.stringify(i.data));
      } else {
        const i = await axios.post("http://localhost:1011/passwordreset", {
          email: email,
        });
        localStorage.setItem("otp", JSON.stringify(i.data));
      }
